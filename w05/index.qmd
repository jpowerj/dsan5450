...
| | Start | End | Topic |
|:- |:- |:- |:- |
| **Lecture** | 6:30pm | 6:50pm | [Setting the Table: HW1 $\leadsto$ HW2 &rarr;](#roadmap) |
...
* *Input 1 (!)*: Attributes $X_{\text{advantage}}$ that a society (real or hypothetical) considers **normatively relevant** for an outcome, but that people are not **individually responsible** for (e.g., race or nationality via birth lottery)
* *Input 2*: Attributes $X_{\text{merit}}$ that a society considers people **individually responsible** for (e.g., effort, sacrificing short-term pleasure for longer-term benefits, etc.)
...
* <i class='bi bi-2-circle'></i>: Individuals' **context-sensitive scores** are computed **relative to their group** $S_i$, as $z$-score of their $X_{\text{merit}}$ value relative to **distribution** of $X_{\text{merit}}$ values across $S_i$
...
## Causal Fairness in HW2 {.crunch-title .inline-90}
